...
struct node                    // to store info of a node
{
        ...
        double f_dist;         // stores the f = g + h value
};

bool operator<(const node &n1,const node &n2) // for priority queue
{
        return n1.f_dist>n2.f_dist;  // for A*, f value is compared
}
...
void A_star_plan(Mat map,Point start,Point dest)  // x  ---> cols   y ---> rows   i ---> cols  j ---> rows
{
        ...
        nodes[start.y][start.x].f_dist = 0;
        ...
        while(!Q.empty())  
        {
                ...
                for(int i = curr_parent.coord.x-1;i<curr_parent.coord.x+2;i++)      // iterate in a kernel 3X3
                {
                        for(int j = curr_parent.coord.y-1;j<curr_parent.coord.y+2;j++)
                        {
                                ...
                                double heur = heuristic(dest,nodes[j][i].coord);
                                double f_value = nodes[curr_parent.coord.y][curr_parent.coord.x].dist + cost + heur; // f = g + h                               
                                if(f_value < nodes[j][i].f_dist)// f condition 
                                {
                                        ...
                                        nodes[j][i].f_dist = f_value; // update the f value
                                        Q.push(nodes[j][i]); 
                                }
...
int main(int argc, char **argv)
{
        ...
        tic = clock();
        // call A* to make path
        A_star_plan(map,start,destination);
