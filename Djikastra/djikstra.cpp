...
void plan(Mat img,Point source,Point goal) {
	...
	vector<vector<Node> > parent(img.cols,vector<Node>(img.rows,Node(source.x,source.y,0)));
	vector<vector<double> > distance(img.cols,vector<double>(img.rows,FLT_MAX));

	cout << "Nodes pushed\n";

	Node src(source.x,source.y,0);
	Q.push(src); 
	distance[source.x][source.y] = 0;
	parent[source.x][source.y] = Node(source.x,source.y,0);
				...
				if(isValid(current.p.x + k,current.p.y + l,obstacles)) {
					// if(visited[current.p.x + k][current.p.y + l] == 0) {
						...
						if(distance[current.p.x][current.p.y] + wt < distance[current.p.x + k][current.p.y + l]) {
							...
							parent[current.p.x + k][current.p.y + l] = Node(current.p.x,current.p.y,0); //distance here dose'nt matter
							...
							Q.push(Node(current.p.x + k,current.p.y + l,distance[current.p.x][current.p.y] + wt));
				}
			// }
			}
	...
	Node current = Node(goal.x,goal.y,0);
	Node cur_next = Node(goal.x,goal.y,0);
	...
	while(1) {
		...
		//cout << "ANDAR" << endl;
		current = Node(cur_next.p.x,cur_next.p.y,0);
		img.at<uchar>(current.p.y,current.p.x) = 255;
		// cout << x_cur << "," << y_cur << endl;
		cur_next = Node(parent[current.p.x][current.p.y].p.x,parent[current.p.x][current.p.y].p.y,0);
		x_cur = cur_next.p.x;
		y_cur = cur_next.p.y;
		// imshow("DJI",img);
		// waitKey(10);
