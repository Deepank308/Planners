...
void rrt_plan(Mat map_org,node start,node destination)
{
        ...
        while(not_reached)                // loop until not reached
        {
                ...
                if(!IsValid(map_org,Point(q.x,q.y))) continue;   // check if feasible
              ...
              //  int point_in_reach = 0;
                ...
                if(!find_reachable_node(map_org,&q,rrt_tree[min_dist_index])) continue;   // search for reachable node along the line
                map_org.at<uchar>(q.y,q.x) = 255;   // mark the node as visited
             //   circle(map, Point(q.x,q.y),3,Scalar(255,0,0),3,LINE_8,0 );
                line(map,Point(q.x,q.y),Point(rrt_tree[min_dist_index].x,rrt_tree[min_dist_index].y),Scalar(0,150,0),3,LINE_8,0 );
                q.parent_index = min_dist_index;   // parent is the node closest to it
